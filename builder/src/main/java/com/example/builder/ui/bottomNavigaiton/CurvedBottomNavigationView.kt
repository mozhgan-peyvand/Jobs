...
class CurvedBottomNavigationView  constructor(
    context: Context,
    ...
    var selectedColor = Color.parseColor("#6433B9")
        set(value) {
            field = value
    ...
    fun setupWithNavController(
        navController1: State<NavBackStackEntry?>,
...
//                if (matchDestination(navController1.value, cbnMenuItems[i].destinationId)) {
...
//        }
    ...
    private fun navigateToDestination(navController: NavController, itemCbn: CbnMenuItem) {
        ...
        val builder = NavOptions.Builder()
            ...
            .setPopExitAnim(androidx.navigation.ui.ktx.R.anim.nav_default_exit_anim)
        // pop to the navigation graph's start  destination
//        builder.setPopUpTo(findStartDestination(navController.graph).id, false)
        val options = builder.build()
        try {
            navController.navigate(itemCbn.destinationId, navOptions =  options)
    ...
    // source code referenced from the actual JetPack Navigation Component
    // refer to the original source code
    private fun matchDestination(destination: NavBackStackEntry?, destinationId: String): Boolean {
//        var currentDestination = destination
//        while (currentDestination?.id.toString() != destinationId && currentDestination?.id.toString() != null) {
...
//
    ...
    private fun findStartDestination(graph: NavGraph): NavDestination {
        ...
        while (startDestination is NavGraph) {
            startDestination = graph.findNode(graph.startDestinationRoute)!!
        }
    ...
    private fun animateItemSelection(offset: Int, width: Int, index: Int) {
        ...
        val halfAnimDuration = animDuration
        // hide the FAB
